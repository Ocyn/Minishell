...
/*   Updated: 2024/02/22 19:21:55 by aammirat         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_export(t_cmd *cmd, t_env *env)
{
	...
	{
		if (test_valid(cmd->meta.raw[i]))
		{
			if (is_in_env(cmd->meta.raw[i], env))
				ft_unset(cmd, env, cmd->meta.raw[i]);
			else
			{
				buf = env;
				while (buf->next)
